...
class SimilarBookPresenterImpl(override var view: SimilarBookContract.View) : SimilarBookPresenter {

    override fun getBookListSimilar(context: Context) {

        BookService().getBookListRecentSimilar(object : RetrofitCallback<SingleResult<BookRecentSimilarListResDTO>> {
            override fun onSuccess(code: Int, responseData: SingleResult<BookRecentSimilarListResDTO>) {
                if (responseData.output==1) {
                    val bookList = responseData
                    bookRecentAdapter = RecentBookAdapter()
                    bookRecentAdapter!!.submitList(bookList.data.bookRecentSimilarList)
                    view.bindInfo()
                } else {
